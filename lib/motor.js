  ...
  EV3: {
    ...
    setPWM: {
      value: function(pin, value) {
        ...
        var register = state.shield.motor === EV3.M1 ? EV3.SPEED_M1 : EV3.SPEED_M2;
        var speed = __.scale(value, 0, 255, 0, 100) | 0;

        if (value === 0) {
          state.ev3.write(state.shield, EV3.COMMAND, EV3.Motor_Reset);
        } else {
          ...
          var data = [
            ...
            0,
            // Command A
            EV3.CONTROL_SPEED | EV3.CONTROL_GO
          ];
...
Controllers.NXT = Controllers.EV3;
...
function Motor(opts) {
  ...
  if (typeof this.opts.device === "undefined") {
    ...
    if (this.opts.controller === "EV3" || this.opts.controller === "NXT") {
